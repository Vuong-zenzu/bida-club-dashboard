...
export const TransferModal: React.FC<TransferModalProps> = ({ isOpen, onClose, sourceTable }) => {
    ...
    const { tables, transferSession } = useTableContext();
    const [selectedTarget, setSelectedTarget] = useState<string | null>(null);

    // Get real available tables (excluding source table and occupied tables)
    const availableTables = tables
        .filter(t => t.id !== sourceTable && t.status === 'AVAILABLE')
        .map(t => t.id);

    const handleConfirm = () => {
        if (selectedTarget) {
            transferSession(sourceTable, selectedTarget);
            onClose();
        }
    };
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm animate-in fade-in">
            <div
                ...
                style={{ backgroundColor: theme.bgCard, border: `1px solid ${theme.primary}40` }}
            >
                {/* Header */}
                ...
                <div className="flex items-center justify-between mb-8 px-4">
                    ...
                    <div className="text-center">
                        ...
                        <div
                            ...
                            style={{
                                backgroundColor: selectedTarget ? `${theme.primary}20` : 'transparent',
                                borderColor: selectedTarget ? theme.primary : 'rgba(255,255,255,0.1)',
                                color: selectedTarget ? theme.primary : 'rgba(255,255,255,0.2)'
                            }}
                        >
                            {selectedTarget || '?'}
                        </div>
                ...
                `}
                                style={{
                                    backgroundColor: selectedTarget === tId ? theme.primary : undefined,
                                    borderColor: selectedTarget === tId ? theme.primary : undefined
                                }}
                            >
                                {tId}
                            </button>
                ...
                <div className="flex gap-3">
                    ...
                    <button
                        ...
                        style={{
                            backgroundColor: selectedTarget ? theme.primary : 'rgba(255,255,255,0.1)',
                            color: selectedTarget ? theme.bgPrimary : 'rgba(255,255,255,0.3)'
                        }}
                    >
                        Xác nhận
                    </button>
